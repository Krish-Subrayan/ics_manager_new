...
require_once('SG-iCalendar-master/SG_iCal.php');

function dump_t($x) {
	echo "<pre>".print_r($x,true)."</pre>";
}
$ICS = $_SESSION['user_url'];
...
foreach($data as $key => $val){
  ...
  $dis.='<div class="col-sm-4"><div class="card-block">
			...
			<div class="card-footer text-muted">
				<p><small><strong>Start Time:</strong> '. date('m/d/Y h:i:s a', $val['start'])
.'</small></p>
				<p><small><strong>End Time:</strong>'. date('m/d/Y h:i:s a', $val['end']).'</small></p>
			</div>
			</div>
  ';
